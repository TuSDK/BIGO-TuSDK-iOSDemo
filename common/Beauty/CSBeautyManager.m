...
- (void)setBeautifyFilterWithPath:(NSString * _Nullable)filterPath level:(int)level {
    ...
    [[CStoreMediaEngineCore sharedSingleton] sendMediaSideInfo:[NSString stringWithFormat:@"ios:filter is changed,send in audio."]];
}
...
- (void)setStickerWithPath:(NSString * _Nullable)path {
    ...
    [[CStoreMediaEngineCore sharedSingleton] sendMediaSideInfo:[NSString stringWithFormat:@"ios:sticker is changed,send in video."]];
}
