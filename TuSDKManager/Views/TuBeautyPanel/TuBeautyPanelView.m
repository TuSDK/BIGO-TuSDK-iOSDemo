...
- (void)commonInit
{
    ...
    tabbar.itemTitles = @[NSLocalizedStringFromTable(@"tu_美肤", @"TuSDKConstants", @"美肤"), NSLocalizedStringFromTable(@"tu_微整形", @"TuSDKConstants", @"微整形"), NSLocalizedStringFromTable(@"tu_美妆", @"TuSDKConstants", @"美妆")];
...
- (void)tuFacePlasticPanelView:(TuFacePlasticPanelView *)view didSelectCode:(NSString *)code
{
    ...
    {
        NSString *title = NSLocalizedStringFromTable(@"tu_重置", @"TuSDKConstants", @"重置");
        NSString *msg = NSLocalizedStringFromTable(@"tu_将所有参数恢复默认吗？", @"TuSDKConstants", @"将所有参数恢复默认吗？");
        ...
        [alert addAction:[TuAlertAction actionWithTitle:NSLocalizedStringFromTable(@"tu_取消", @"TuSDKConstants", @"取消") handler:^(TuAlertAction * _Nonnull action)
        {
            self->_paramtersAdjustView.hidden = YES;
        }]];
        
        [alert addAction:[TuAlertAction actionWithTitle:NSLocalizedStringFromTable(@"tu_确定", @"TuSDKConstants", @"确定") handler:^(TuAlertAction * _Nonnull action)
        {
...
- (void)cosmeticParamtersViewUpdate:(NSString *)code
{
    ...
    for (int cosmeticIndex = 0; cosmeticIndex < _cosmeticParams.args.count; cosmeticIndex++)
    {
        ...
        {
            NSString *paramName = [NSString stringWithFormat:@"lsq_filter_set_%@", code];
            paramName = NSLocalizedStringFromTable(@"tu_不透明度", @"TuSDKConstants", @"无需国际化");
