...
@interface TuSDKManager()<TuFilterPanelViewDelegate, TuStickerPanelViewDelegate, TuBeautyPanelViewDelegate, SelesParametersListener>
{
    UIView *_superView;
...
- (void)resetAllCosmeticAction
{
    ...
    NSString *title = NSLocalizedStringFromTable(@"tu_美妆", @"TuSDKConstants", @"美妆");
    NSString *msg = NSLocalizedStringFromTable(@"tu_确定删除所有美妆效果?", @"TuSDKConstants", @"美妆");
    ...
    UIAlertAction *confimAction = [UIAlertAction actionWithTitle:NSLocalizedStringFromTable(@"tu_确定", @"TuSDKConstants", @"确定") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
#pragma mark - TuSDKFilterPipe output
- (CVPixelBufferRef)syncProcessSampleBuffer:(CMSampleBufferRef)sampleBuffer
{
    ...
    CVPixelBufferRef convertBuffer = [self convertPixelBufferToYUVData:newPixelBuffer];
    
    return convertBuffer;
}

- (CVPixelBufferRef)convertPixelBufferToYUVData:(CVPixelBufferRef)pixelBuffer
{
    CVPixelBufferLockBaseAddress(pixelBuffer, 0);
    //获取图像宽度
    size_t dst_width = CVPixelBufferGetWidth(pixelBuffer);
    //获取图像高度
    size_t dst_height = CVPixelBufferGetHeight(pixelBuffer);
    
    size_t half_width = (dst_width + 1) / 2;
    size_t half_height = (dst_height + 1) / 2;
    
    const size_t y_size = dst_width * dst_height;
    const size_t uv_size = half_width * half_height;
    
    // YUV
    uint8_t *yBytes = malloc(y_size + 2 *uv_size);
    uint8_t *uvBytes = yBytes + y_size;
    
    uint8_t *data = CVPixelBufferGetBaseAddress(pixelBuffer);
    size_t bgraStride = CVPixelBufferGetBytesPerRow(pixelBuffer);
    
    ARGBToNV12(data, bgraStride, yBytes, dst_width, uvBytes, dst_width, dst_width, dst_height);
    ...
    planeWidths[0] = dst_width;
    planeWidths[1] = dst_width;
    
    size_t planeHeights[2];
    planeHeights[0] = dst_height;
    planeHeights[1] = dst_height / 2;
    
    uint8_t *baseAddresses[2];
    baseAddresses[0] = yBytes;
    baseAddresses[1] = uvBytes;
    
    CVReturn result = CVPixelBufferCreateWithPlanarBytes(kCFAllocatorDefault, dst_width, dst_height, kCVPixelFormatType_420YpCbCr8BiPlanarFullRange, yBytes, uv_size + y_size, 2, (void *)baseAddresses, planeWidths, planeHeights, planeWidths, releasePlanarBytesCallback, NULL, NULL, &newPixelBuffer);
